...
## v3: 자동 발행 모드 (중요)
대행사 데이터는 매번 같은 시각에 올라오지 않습니다:
- 월~목 데이터: 그 다음날 오후 1시(KST)
- 금/토/일 데이터: 모두 그 다음 월요일 오후 1시(KST)에 한꺼번에 올라옴

그래서 "매일 정해진 시간에 실행"만으로는 부족합니다 — 하루 실행에서 발행해야 할 날짜가
0개, 1개, 3개(월요일)로 매번 달라지기 때문입니다. `--date` 없이 실행하면 스크립트가
직접 판단합니다:
1. 지금(KST) 기준으로 위 규칙에 따라 데이터가 이미 올라왔을 날짜를 계산
2. 노션 DB를 조회해서 그중 아직 페이지가 없는 날짜만 골라냄
3. 골라낸 날짜를 오래된 순으로 하나씩 발행 (루틴이 하루 빠뜨려도 다음 실행에서 자동으로 따라잡음)

`--date YYYY-MM-DD`를 주면 이 판단을 건너뛰고 그 날짜 하나만 강제로 발행합니다(백필/테스트용).
...
6. Trigger: Scheduled > Daily, **오후 1시 30분(KST)** 근처로 설정 (데이터가 1시에 올라오니
   30분 정도 여유를 둔다 — 라우틴 설정 화면의 시간대 표기를 확인해서 KST로 맞출 것)
7. 저장 전에 **Run now**로 먼저 한 번 수동 실행해서 결과 확인 (자동 판단 로직을 건너뛰고
   과거 날짜 하나만 테스트하려면 `python daily_report.py --date 2026-09-20`)
