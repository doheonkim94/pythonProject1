...
def fetch_mediamix(service, target_date):
    ...
    idx_media, idx_product, idx_budget, idx_share, idx_cpi, idx_cpa = (
        col("매체"),
        col("상품"),
        col("광고비"),  # '예산'은 '예산 비중'(%) 열에도 걸려서 실제 금액 열인 '광고비'로 지정
        col("비중"),
        col("CPI"),
        col("CPA"),
    )
...
def build_children(target_date, daily, mediamix, high, low):
    def g(row, col):
    ...
    if mediamix:
        ...
        children.append(
            table(
                ["매체", "상품", "예산", "비중", "예상 CPI", "예상 CPA"],
                [[r["매체"], r["상품"], r["예산"], r["비중"], r["CPI"], r["CPA"]] for r in mediamix],
            )
        )
    else:
    ...
    children.append(para("현재 확보된 소재 성과 원본은 매체 필터가 고정돼 있어, 필터를 바꾸지 않는 한 다른 매체의 소재별 성과는 이 리포트에 포함되지 않습니다."))

    children.append(h2("6. 개선점"))
    if low:
        names = ", ".join(c["name"] for c in low)
        children.append(bullet(f"저효율 소재 정리 검토 대상: {names}"))
    if high:
        top = high[0]
        children.append(bullet(f"고효율 패턴 확장 후보: {top['name']} (CPA {top['cpa']}원, USP: {top['usp']})"))
    children.append(bullet("매체별 실적 데이터가 없어 채널 간 예산 재배분 판단이 어렵습니다 — 매체 API 연동을 우선 검토하세요."))
...
def main():
    ...
    mediamix = fetch_mediamix(sheets, target_date)
    ...
    children = build_children(target_date, daily, mediamix, high, low)
